...
public class TicTacTest {
	...
	public static void main(String args[]) {
		boolean boolPositionValue[][] = { { false, false, false }, { false, false, false }, { false, false, false } };
		System.out.println("Refer below board for entering your values");
		String arr[][] = { { "11", "12", "13" }, { "21", "22", "23" }, { "31", "32", "33" } };
		displayGrid(arr);
		boolean win = true;
		int turns = 1;
		...
		while (win && turns <= 9) {
			...
			boolean posValue = checkPositionIsFilled(boolPositionValue[row][col]);
			if (posValue == false) {
				assignValue(input, row, col, arr, boolPositionValue);
				displayGrid(arr);
				win = checkWin(arr);
				if (win == false) {
